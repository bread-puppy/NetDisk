...
bool CMysql::ExecuteTransaction(const vector<string>& sqlList)
{
    pthread_mutex_lock(&m_lock);
    bool ok = (mysql_query(conn, "START TRANSACTION") == 0);
    for (size_t i = 0; ok && i < sqlList.size(); ++i)
        ok = (mysql_query(conn, sqlList[i].c_str()) == 0);
    if (ok)
        ok = (mysql_query(conn, "COMMIT") == 0);
    else
