...
//心跳协议：客户端定时发送请求，服务端返回相同序号确认连接仍然可用#define _DEF_PACK_HEARTBEAT_RQ      (_DEF_PACK_BASE + 36)
...
//四路 TCP 分片上传协议：先绑定连接，再提交本连接负责的文件区间#define _DEF_PACK_BIND_POOL_RQ      (_DEF_PACK_BASE + 30)
...
struct STRU_CHUNK_UPLOAD_RQ
{
    ...
    int segSize;
    int totalSize;
    int segProgress; // 该分片已经写入的字节数，用于断点续传
...
struct STRU_CHUNK_UPLOAD_RS
{
    STRU_CHUNK_UPLOAD_RS():type(_DEF_PACK_CHUNK_UPLOAD_RS),
        userid(0), fileid(0), segOffset(0), result(0){}
    PackType type;
    int userid;
    int fileid;
    int segOffset;
...
struct STRU_CHUNK_DOWNLOAD_RQ {
    STRU_CHUNK_DOWNLOAD_RQ():type(_DEF_PACK_CHUNK_DOWNLOAD_RQ),userid(0),fileid(0),timestamp(0),segOffset(0),segSize(0){ memset(dir,0,sizeof(dir)); }
    PackType type; int userid; int fileid; int timestamp; int segOffset; int segSize; char dir[_MAX_PATH_SIZE];
};
struct STRU_CHUNK_DOWNLOAD_RS {
    ...
    PackType type; int userid; int fileid; int timestamp; int segOffset; int seq; int len; int result; char content[_DEF_BUFFER];
};
...
{
    //需要 结果 , 用户的id    STRU_LOGIN_RS(): type(_DEF_PACK_LOGIN_RS) , result(login_success),userid(0)
    {
        memset( name , 0, sizeof(name) );
    }
    PackType type;
...
//收到回复 就刷新分享列表struct STRU_SHARE_FILE_RS
{
...
//服务器中 map 有和没有  有 timestamp  没有 需要唯一确认文件 uid fid fdir//【MD5校验】新增 md5 字段：客户端本地已下载部分的MD5，服务端据此做续传一致性校验
...
/* =====================================================================
 * 【新增代码】以下为客户端新增功能对应的协议定义与结构体
 * （搜索 / 收藏 / 回收站 / 浏览分享 / 下载分享文件 / 回收站彻底删除）。
 * 为保持与客户端 packdef.h 完全一致，逐字段复制客户端的结构体定义。
 * 本段全部为追加内容，未改动上方任何原有代码。
 * ===================================================================== */
#define _DEF_SHARE_PWD_LEN  20

//========== 文件搜索协议 ==========#define _DEF_PACK_SEARCH_FILE_RQ    (_DEF_PACK_BASE + 40)
...
//========== 收藏文件协议 ==========#define _DEF_PACK_FAVORITE_FILE_RQ    (_DEF_PACK_BASE + 46)
...
//========== 回收站协议 ==========#define _DEF_PACK_RECYCLE_FILE_RQ    (_DEF_PACK_BASE + 50)
...
//========== 浏览分享目录协议 ==========#define _DEF_PACK_BROWSE_SHARE_RQ   (_DEF_PACK_BASE + 60)
...
//========== 下载分享文件协议（用于预览） ==========#define _DEF_PACK_DOWNLOAD_SHARE_FILE_RQ   (_DEF_PACK_BASE + 62)
...
//========== 回收站彻底删除协议 ==========#define _DEF_PACK_DELETE_FOREVER_RQ   (_DEF_PACK_BASE + 64)
