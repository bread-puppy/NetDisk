...
{
public:
    Block_Epoll_Net(){ pthread_mutex_init(&m_activityLock, NULL); }
    ~Block_Epoll_Net(){}
    //由于服务器 不会主动关闭, 没有写回收
public:
    ...
    /*每连接发送锁：
     * 线程池中多个线程可能同时向同一连接发包（如下载文件块与心跳回包），
     * 两个 send 的字节在 socket 缓冲区中交错，客户端拆包错位 -> 下载文件损坏、
     * 视频无法播放。SendData/SendFileContentZeroCopy 发送前加锁、发完解锁，
     * 保证一个连接上同一时刻只有一个完整的包在发送。
     * 锁对象为每个 fd 惰性创建，连接关闭后保留复用（数量级为历史连接数，可忽略）。
     */
    void SendLock(int fd);
    void SendUnlock(int fd);
    //连接池连接标记：绑定过用户的四路分片连接不受心跳超时清理
    void MarkPoolFd(int fd);
...
private:
    ...
    //检查连接最后活动时间，超时连接由现有收包线程统一清理
    void CheckHeartbeatTimeout();
    void RemoveActivity(int fd);
    //监听套接字对应的是事件
    myevent_s * m_listenEv;
    //监听套接字
    int m_listenfd;
    //epoll_create 句柄    int m_epoll_fd;

    //每一个套接字 对应一个事件结构
    /*map*/MyMap< int , myevent_s*> m_mapSockfdToEvent;
    pthread_mutex_t m_activityLock;
    map<int, time_t> m_lastActivity;
    MyMap<int, pthread_mutex_t*> m_mapSockfdToSendLock;
    //连接池（分片传输）连接集合，与 m_activityLock 共用同一把锁保护
    map<int, bool> m_poolFds;
    /* 事件循环使用 */
