...
int thread_pool::Producer_add( void *(*task)(void *arg),void *arg)
{
	...
	//当前任务队列已满，且线程池未关闭    while(m_pool->queue_cur == m_pool->queue_max && m_pool->thread_shutdown  )
    {
		//队列已满，等待有任务被取出        pthread_cond_wait(&m_pool->not_full,&m_pool->lock);
    }
	//线程池已关闭    if(!m_pool->thread_shutdown  )
    {
        pthread_mutex_unlock(&m_pool->lock); 
        return -1; //退出
    }
	//将任务加入任务队列    m_pool->queue_task[m_pool->queue_front].task = task;
    m_pool->queue_task[m_pool->queue_front].arg = arg;
	//更新队列头指针    m_pool->queue_front = (m_pool->queue_front + 1) % m_pool->queue_max;
	//当前任务队列中的任务数加一    ++(m_pool->queue_cur);
	//通知消费者线程有新的任务可取    pthread_cond_signal(&m_pool->not_empty);
...
{
	//获取线程池对象指针    pool_t * p = (pool_t*)arg;
    task_t task;
    while(p->thread_shutdown)  // 线程池未关闭
    {
        pthread_mutex_lock(&p->lock);// 上锁
		//当前任务队列中没有任务，且线程池未关闭        while(p->queue_cur == 0 && p->thread_shutdown  )
        {
			//等待有新的任务被添加到队列中            pthread_cond_wait(&p->not_empty,&p->lock);
        }
		//线程池已关闭        if(!p->thread_shutdown  )
        {
            pthread_mutex_unlock(&p->lock);
            pthread_exit(NULL); // 结束此线程
        }
		//需要等待关闭的线程数大于零，且当前存活的线程数大于最小线程数        if(p->thread_wait > 0 && p->thread_alive > p->thread_min)
        {
		...
		//取出任务        task.task = p->queue_task[p->queue_rear].task;
    ...
    while(p->thread_shutdown ) // 线程池未关闭
    {
                ...
                p->thread_max > alive)
        {
            for(int j = 0;j<p->thread_min;j++)
            {
                ...
                {
					//该线程不存在或已经结束                    if(p->tids[i] == 0 || !if_thread_alive(p->tids[i]))
                    {
                        pthread_mutex_lock(&p->lock);// 上锁
						//创建新的工作线程                        pthread_create(&p->tids[i],NULL,Custom,(void*)p);
						//存活的线程数加一                        ++(p->thread_alive);
                        pthread_mutex_unlock(&p->lock);// 解锁
                        break;
                    }
                }
